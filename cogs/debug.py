...
class Debug(commands.Cog):
    ...
    async def restart(self, ctx):
        if ctx.author.id not in self.DEBUG_USERS:
            await ctx.respond("Du hast keine Berechtigung für diesen Befehl.", ephemeral=True)
            return
        await ctx.respond("Bot wird neugestartet ...", ephemeral=True)
        await asyncio.sleep(1)
        os.execv(sys.executable, [sys.executable, *sys.argv])

    @discord.slash_command(name="dashboard_urls", description="Get dashboard access URLs")
    async def dashboard_urls(self, ctx):
        if ctx.author.id not in self.DEBUG_USERS:
            await ctx.respond("Du hast keine Berechtigung für diesen Befehl.", ephemeral=True)
            return
        ...
        if not dashboard_cog:
            await ctx.respond("Dashboard Cog nicht gefunden!", ephemeral=True)
            return
        
        urls = await dashboard_cog.dashboard_access_urls()
        lan_line = f"**Network:** {urls['lan']}" if urls.get("lan") else "**Network:** not detected"
        public_line = f"**Public:** {urls['public']}" if urls.get("public") else "**Public:** not configured"
        
        message = f"**Local:** {urls['local']}\n{lan_line}\n{public_line}"
